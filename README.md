...
인스타 프로필 링크로 들어오면 **인스타 앱 안에 박힌 브라우저**가 열립니다. 그 환경은
보통 브라우저와 다릅니다 — 특히 **위치 권한이 조용히 실패하는 일이 잦고**, 홈 화면에
추가할 수도 없습니다. 이 앱은 위치가 없으면 아무것도 못 하므로, 들어온 경로를 보고
**지금 이 사람에게 필요한 한 가지**만 첫 화면에 띄웁니다.

| 상황 | 띄우는 것 | 왜 |
| --- | --- | --- |
| 앱 안 브라우저(인스타·카톡·페북·네이버) | 🧭 밖으로 나가는 방법 + [주소 복사하기] | 제일 급합니다. 여기선 위치가 막힙니다 |
| 처음 온 사람 | 30초 사용법 세 줄 | 뭘 하는 앱인지 3초 안에 |
| 다시 온 사람 (iOS 이거나 설치 가능) | 📲 홈 화면에 추가 | 재방문할 사람에게만 권합니다 |
| 이미 홈 화면에서 실행 중 | 아무것도 안 띄움 | 더 알려줄 게 없습니다 |

배너는 **한 번에 하나만** 나옵니다. 위에서부터 먼저 걸리는 것 하나로 끝냅니다
(`src/components/EntryBanner.tsx` 의 `pickBanner`).

닫기를 누르면 사용법·설치 안내는 `localStorage` 에 기억해 다시 띄우지 않습니다.
앱 안 브라우저 경고만은 **이번 방문에서만** 접습니다 — 다음에 또 인스타로 들어오면
또 필요한 정보라서요.

### 유입 표시 붙이기

인스타 프로필의 링크에 `?from=ig` 를 붙이면 "인스타그램에서 오셨네요"까지 말해줍니다.

```
https://<도메인>/?from=ig
```

- 인정하는 값: `ig` `instagram` `threads` `kakao` `kakaotalk` `facebook` `fb` `naver`
- `utm_source` 도 같은 목록으로 읽습니다 (`?utm_source=instagram`)
- 목록에 없는 값은 무시합니다 — 주소창에 아무 말이나 넣어 화면 문구를 바꾸지 못하게
- 표시가 없어도 **UA 와 referrer 로** 인스타/카톡 인앱 브라우저는 알아냅니다.
  표시는 "어디서 왔는지 이름을 불러주기" 위한 덤입니다

판별 순서는 `붙여준 표시 > 앱 안 브라우저 UA > referrer > direct` 입니다
(`src/lib/entry.ts`).

### 위치가 막혔을 때

앱 안 브라우저에서 "현재 위치 사용"이 실패하면, 위치 시트가 그냥 "설정을
확인하세요"라고 하지 않습니다 — 확인할 설정이 없어 막다른 길이 되니까요.
원인(앱 안 브라우저라서)과 빠져나가는 방법, 그리고 **검색으로 정해도 된다는 것**을
같이 알려줍니다.

### 홈 화면에 추가 (PWA)

`public/manifest.webmanifest` + `public/icon.svg` 가 있어 홈 화면에 추가하면 주소창
없이 앱처럼 열립니다. 안드로이드/크롬은 `beforeinstallprompt` 를 잡아뒀다가
[홈 화면에 추가] 버튼으로 바로 띄우고(`src/store/installStore.ts`), iOS 는 그 이벤트가
없으므로 공유 버튼 위치를 글로 알려줍니다.

서비스 워커는 아직 없습니다 — 오프라인으로 쓸 만한 앱이 아니고(식당 데이터가 매번
필요합니다), 캐시가 낡으면 디버깅이 더 어려워집니다.
...
| 🎯 **음식 다트** | 돌아가는 판에 타이밍 맞춰 던지기 | **3D 판** 회전 + 파워 게이지, 꽂힌 자리 표시 |
| 🕹️ **뽑기 기계** | 집게를 세우고 악력을 맞춰 캡슐 집기 | **3D 기계**, 2단 타이밍, 미끄러짐 |
| 🏃 **음식 달리기** | 밀 곳을 고르고 연타로 결승선까지 | 실시간 레인 막대, 누가 어디를 미는지 |
| 🎣 **음식 낚시** | 헛입질을 참고 진짜에 챔질 | 사람마다 다른 입질, 기회는 한 번 |
| ⚡ **눈치 게임** | 한 장씩 지나가는 후보에 찜 3번 | 3.2초 타이머 막대, 남의 티켓 잔량 공개 |
| 🔮 **이심전심** | 전원이 같은 곳을 고르면 그 자리에서 끝 | 동시 공개 → 최저 득표 탈락 → 다시 |

동점·무투표는 시드로 결정되어 모든 참가자가 같은 결과를 봅니다.

게임마다 노리는 감각이 다릅니다 — **실력(다트)**, 겨루기(배틀), 거부권(지우기),
자원 배분(경매), 속도(눈치), 합의(이심전심), 설득(판사), 운(룰렛·운명).
같은 축이 겹치지 않게 넣습니다.

#### 손으로 하는 게임 — 실력이 결과를 바꿉니다

나머지는 전부 "고르기" 입니다(투표하거나 탭하거나 지우거나). 아래 넷은 손으로 하는
게임이라, 잘하고 못하고가 있습니다.

| 게임 | 실력이란 |
| --- | --- |
| 🎯 다트 | 돌아가는 판(3.6초) + 파워 게이지(1.1초) — **한 번 누르기** |
| 🕹️ 뽑기 | 좌우 이동(2.4초) 세우고, 악력(1초) 맞추기 — **두 번 누르기** |
| 🏃 달리기 | 8초 동안 얼마나 두드리는가 — **지구력** |
| 🎣 낚시 | 헛입질에 손이 나가지 않는가 — **참을성** |

같은 "타이밍" 이라도 다트는 1축, 뽑기는 2축입니다. 달리기는 연타, 낚시는 그 반대로
억제입니다.

* **연타는 한 번씩 보내지 않습니다.** 각자 자기 화면에서 세고 누적 합계만 이따금
  보냅니다 — 늦게 오거나 중복으로 와도 결과가 같습니다. 폴링으로 도는 방이
  연타를 감당할 수 없기 때문입니다.
* **낚시의 입질 순서는 상태에 없습니다.** 사람마다 시드로 계산해서, 옆 사람 화면을
  봐도 소용없고 서버가 진짜/가짜를 다시 계산합니다.
* **뽑기의 집게 움직임은 각자 화면에서만 돕니다.** 남의 집게를 볼 이유가 없고,
  공유되는 건 결과뿐이라 방이 가볍습니다.

#### 다트 자세히 판이 3.6초에 한 바퀴 돌고, 파워 게이지가 1.1초에 한 번 왕복합니다.
**던지는 순간의 판 각도가 겨눈 칸을, 게이지를 얼마나 맞췄는지가 실제로 꽂히는지를**
정합니다. 잘하면 원하는 데를 맞힙니다.

* 판 각도는 시작 시각에서 계산합니다 — 늦게 들어와도, 다른 폰에서도 같은 판입니다.
* **화면과 판정이 같은 공식**(`aimedIndex`)을 씁니다. 눈에 보이는 칸과 실제로 겨눈
  칸이 어긋나면 게임이 아니라 사기입니다.
* 정확도는 클라이언트가 보내지만 서버(호스트)가 0~1 로 자르고, 빗나가는 방향은
  시드로 정합니다. 같은 입력이면 항상 같은 결과입니다.
* 화면에 안 보이는 동안에는 3D 렌더링을 멈춥니다(배터리).
