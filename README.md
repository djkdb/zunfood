...
| 🎲 **혼자 결정하기** | 1명 | 위치 → 조건 → 방식 고르기 → 직접 플레이하거나 바로 받기 |
...
식당이 "예시 데이터" 로 나온다면 여기부터 본다.

```
https://<worker>.workers.dev/status
```

폰에서 열면 무엇이 빠졌고 어디를 눌러야 하는지 한 화면에 나온다. 값이 다 들어와
있으면 버튼 하나로 "카카오가 이 키를 실제로 받아주는지" 까지 확인한다.
키 값은 표시하지 않는다 — 있음/없음과 바인딩 이름만 나온다.

기계가 읽을 형태가 필요하면 `/api/status` 를 쓴다.
...
├─ solo/            혼자 결정하기 (직접 플레이 3종 + 맡기기 5종)
│  └─ games/        월드컵·넘기기·사다리 화면
...
#### 반경을 넓혀도 가까운 곳만 나오던 문제

카카오 로컬 API 는 **한 번의 검색에 반경과 무관하게 최대 45건**(15 × 3페이지)만
돌려줍니다. 가까운 순이라, 반경을 2km 로 넓혀도 실제로는 가장 가까운 300m 남짓만
보게 됩니다.

그래서 중심에서 한 번 훑어보고 45건을 꽉 채웠으면(= 더 있다는 뜻) 반경을 구역으로
나눠 각 구역을 따로 검색합니다. 구역마다 45건 한도가 새로 생기므로 반경 전체가
결과에 들어옵니다.

* 구역 중심은 반경의 50% · 90% 지점에 육각형으로 6개씩, 각 구역은 반경의 45% 를 훑습니다.
* 한산한 동네는 중심 검색만으로 끝나서 호출을 낭비하지 않습니다.
* 결과는 구역끼리 번갈아 담습니다. 합쳐서 거리순으로 자르면 결국 가까운 구역이
  다 차지해서 넓힌 의미가 없어집니다.
* Cloudflare Workers 는 요청당 바깥 호출이 50개로 제한되므로, 카카오 호출은 30개로
  묶어둡니다(구글·DB 몫을 남겨둡니다).
* 카카오가 주는 `distance` 는 검색에 쓴 좌표 기준이라, 구역 검색 결과는 사용자의
  실제 위치로 거리를 다시 계산합니다.

#### 지도에서 위치 고르기 (선택)

핀을 화면 가운데 고정하고 지도를 움직이는 방식입니다. 모바일에서 작은 핀을 정확히
짚는 것보다 쉽고 손가락에 가리지도 않습니다. 검색 반경을 원으로 같이 그려서
"얼마나 넓게 볼지" 가 눈에 보입니다.

쓰려면 **JavaScript 키**가 필요합니다 — REST 키와 다른 키입니다.

1. 카카오 개발자 콘솔 → [앱 설정] → [플랫폼 키] → **JavaScript 키** 복사
2. 같은 콘솔 → [앱 설정] → [플랫폼] → **Web 플랫폼 등록** → 사이트 도메인에
   배포 주소를 추가 (`https://<worker>.workers.dev`). 로컬 개발용으로
   `http://localhost:5173` 도 함께 넣어두면 편합니다.
3. Cloudflare → Settings → Build → Variables 에 `VITE_KAKAO_JS_KEY` 추가 후 재배포

이 키는 **브라우저에 노출되는 게 정상**입니다. 등록한 도메인에서만 동작하는 것으로
보호합니다. 비워두면 "지도에서 고르기" 버튼만 숨고 현재 위치·장소 검색은 그대로
동작합니다. 키가 있는데 도메인을 등록하지 않으면 지도 화면이 그 사실을 알려주고
검색으로 빠져나갈 길을 줍니다.
...
| 게임 | 방식 | 연출 |
| --- | --- | --- |
| 🎰 **음식 룰렛** | 후보 8곳을 룰렛에 올려 한 번에 결정 | 3·2·1 → **3D 휠** 회전 → 바늘 아래 정지 |
...
| 🙅 **지우기** | 돌아가며 한 곳씩 지우고 마지막에 남은 곳으로 | 차례 표시, 15초 제한, 지운 사람 흔적 |

동점·무투표는 시드로 결정되어 모든 참가자가 같은 결과를 봅니다.

게임 목록은 **직접 개입하는 게임이 앞**에 옵니다. 눌러놓고 구경만 하는 게임이
먼저 보이면 앱 전체가 "그냥 랜덤 뽑기" 로 읽힙니다.

#### 3D 룰렛

`three.js` 로 그립니다. 다만 **결과는 이미 시드로 정해져 있고** 3D 는 그 결과에
정확히 멈추도록 보여주기만 합니다 — 물리 시뮬레이션을 쓰면 기기마다 다른 칸에
멈춰 "모두가 같은 결과를 본다" 는 약속이 깨집니다.

* three.js 는 룰렛을 실제로 볼 때만 내려받습니다(별도 청크 ~119KB gzip).
  다른 게임을 고른 사람은 값을 치르지 않습니다.
* 한글을 3D 안에 세우려면 큰 폰트 파일이 필요해서, 판을 캔버스에 한 번 그려
  텍스처로 입힙니다. 폰트를 더 받지 않고도 또렷하게 나옵니다.
* 글자는 호(arc) 방향으로 눕히고, **멈출 각도를 미리 알고** 그 자리에서 바로
  읽히도록 뒤집어 그립니다. 당첨 칸은 항상 12시에 서므로 그 칸이 제일 잘 읽혀야 합니다.
* WebGL 이 없거나 청크를 못 받거나 `prefers-reduced-motion` 이면 기존 2D 룰렛으로
  내려갑니다. 룰렛이 안 돌면 방 전체가 멈추기 때문입니다.

### 혼자 결정하는 방식

**직접 골라서 정하기** — 결과를 내가 만든다

| 방식 | 하는 일 |
| --- | --- |
| 🏆 **음식 월드컵** | 8강 → 4강 → 결승. 둘 중 하나씩 고르면 끝까지 올라온 곳이 오늘의 한 끼 |
| 💘 **넘기기** | 한 장씩 보여주고 ✕/❤️. 정보가 한 곳뿐이라 판단이 빠르다 |
| 🪜 **사다리 타기** | 줄을 하나 고르면 길이 그려지며 내려간다. 사다리는 시드로 미리 정해져 있다 |

**맡기고 바로 받기** — 누르면 끝

🎰 랜덤 룰렛 · 🍀 오늘의 운명(날짜 고정) · 🎲 카테고리 뽑기 · 🎯 조건 추천 · 🔥 근처 인기

월드컵·넘기기는 종류가 골고루 섞이도록 후보를 고릅니다(`diversePool`). 가까운 순으로
그냥 자르면 같은 골목의 비슷한 가게만 올라와서 고를 맛이 없습니다.
...
* **AI 판사** — 결정론적 로컬 판결. `VITE_AI_JUDGE_ENDPOINT` 로 실제 LLM 서버 연결 가능.
  ...
  씁니다. 없는 값을 지어내지 않습니다.
* **지도** — 위치 고르기에 카카오 지도 SDK 를 씁니다(아래). 식당 상세·길찾기는
  외부 지도 링크로 이동합니다.
